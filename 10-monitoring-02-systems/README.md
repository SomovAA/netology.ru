...
Компенсировать можно при помощи отправки этого документа ему на почту, либо предложить в письме бесплатную генерацию документов на время или по купону.
Либо вернуть часть денег на счёт, или пополнить счет, можно продлить подписку и т.п. Это всё больше связано с маркетингом, 
не совсем понимаю, как это соотносится с devops.

SLO: Можно ограничить не только отношением успешных запросов, к неуспешным...но и по времени генерации документа, и по времени отдачи.
Таким образом можно будет найти отношения: удовлетворяющие по времени и превышающие генерацию документа или его отдачу, 
разделив каждое на общее количество. Так мы поймем процент успеха и провала. Можно будет и среднее значение найти.
Формулы вычислений как раз будут в SLI, а тут мы говорим об ограничении, и какие вычисления можно будет совершить.

SLI: = count_good_time/count_all_time.
Где count_good_time - количество удовлетворяющих запросов, связанных с рассчетом времени, 
count_all_time - количество всех запросов, связанных с рассчетом времени
```
...
```
Перепробовал всё возможное из https://github.com/influxdata/telegraf/tree/master/plugins/inputs/docker и с привилегированным пользователем, не выдает информацию о докере
```
